...
func addDeployJobIfSet(config *apps.Config, job deployJob) {
	if len(job.Command) != 0 && len(job.Name) != 0 {
		...
		config.DeployJob = deployJob
	}
}

func addWorkerJobIfSet(config *apps.Config, job workerJob) {
	if len(job.Command) != 0 && len(job.Name) != 0 {
...
func addScheduledJobIfSet(config *apps.Config, job scheduledJob) {
...
func newConfig(bc baseConfig) apps.Config {
	...
	addDeployJobIfSet(&config, bc.DeployJob)
	addWorkerJobIfSet(&config, bc.WorkerJob)
	addScheduledJobIfSet(&config, bc.ScheduledJob)
