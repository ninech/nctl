...
func equalAsSets(a, b []string) bool {
	return maps.Equal(setFromSliceTrimmed(a), setFromSliceTrimmed(b))
}
...
func equalPermissions(a, b []*storage.BucketPermission) bool {
	if len(a) != len(b) {
		return false
	}
	for i := range a {
		if string(a[i].Role) != string(b[i].Role) {
			return false
		}
		if len(a[i].BucketUserRefs) != len(b[i].BucketUserRefs) {
			return false
		}
		for j := range a[i].BucketUserRefs {
			if a[i].BucketUserRefs[j] == nil || b[i].BucketUserRefs[j] == nil {
				if a[i].BucketUserRefs[j] != b[i].BucketUserRefs[j] {
					return false
				}
				continue
			}
			if a[i].BucketUserRefs[j].Name != b[i].BucketUserRefs[j].Name {
				return false
			}
		}
	}
...
// If the resulting permissions are identical to the input `base`, the function
// returns the original slice and `changed=false`. Otherwise a new slice and
// `changed=true` are returned.
//
// Returns an error if role names are invalid, if empty users appear in `add`,
// or if parsing of the specs fails.
func PatchPermissions(base []*storage.BucketPermission, add, remove []string) ([]*storage.BucketPermission, bool, error) {
	...
	if err != nil {
		return nil, false, fmt.Errorf("add permissions: %w", err)
	}
	removeSpecs, err := parsePermissionSpecs(remove, true)
	if err != nil {
		return nil, false, fmt.Errorf("remove permissions: %w", err)
	}
	...
	for _, spec := range addSpecs {
		...
		for _, u := range spec.Users {
			...
			if u == "" {
				return nil, false, fmt.Errorf("add permissions: empty %s for %s %q", permKeyUser, PermKeyRole, role)
			}
			set.Add(u)
		}
	}
	...
	for _, spec := range removeSpecs {
		...
		for _, u := range spec.Users {
			u = strings.TrimSpace(u)
			if u == "" {
				return nil, false, fmt.Errorf("remove permissions: empty %s for %s %q", permKeyUser, PermKeyRole, role)
			}
			set.Del(u)
		}

		// Note: do NOT delete the role if it becomes empty here.
		// Keeping the (possibly empty) role is intentional.
		// if len(set) == 0 {
		// 	delete(roleToUsers, role)
		// }
	}
...
// Compare two slices regardless of order by mapping on Prefix. If the same
// Prefix appears multiple times (shouldn't after our normalization), last one
// wins before comparison.
func equalLifecyclePolicies(a, b []*storage.BucketLifecyclePolicy) bool {
	if len(a) != len(b) {
		// quick fail, but still need content compare in case both contain
		// duplicates - however our normalization ensures unique prefixes, so this
		// is safe.
		return false
	}

	index := func(in []*storage.BucketLifecyclePolicy) map[string]*storage.BucketLifecyclePolicy {
		m := make(map[string]*storage.BucketLifecyclePolicy, len(in))
		for _, p := range in {
			if p == nil {
				continue
			}
			// last-wins if duplicates appear
			m[p.Prefix] = p
		}
		return m
	}
...
func equalLifecyclePolicy(a, b *storage.BucketLifecyclePolicy) bool {
	if a == nil && b == nil {
		return true
	}
	if a == nil || b == nil {
	...
	if a.ExpireAfterDays != b.ExpireAfterDays {
		return false
	}
...
//
// The result is a deterministic, prefix-sorted slice. If the final set is
// identical to the input, the original slice is returned unchanged to preserve
...
func PatchLifecyclePolicies(
	...
	addChunks []string,
	deleteChunks []string,
) ([]*storage.BucketLifecyclePolicy, bool, error) {
	...
	if len(addChunks) > 0 {
		for idx, raw := range addChunks {
			...
			if err != nil {
				return nil, false, fmt.Errorf("lifecycle-policy #%d parsing error: %w", idx+1, err)
			}
			prefix := strings.TrimSpace(spec.Prefix)
			if prefix == "" {
				return nil, false, fmt.Errorf("lifecycle-policy #%d: missing required key %q", idx+1, "prefix")
			}
	...
	// Preserve original identity if unchanged (nil vs empty semantics).
	if equalLifecyclePolicies(base, out) {
		return base, false, nil
	}
	return out, true, nil
}
...
func equalCORS(a, b *storage.CORSConfig) bool {
	if a == nil && b == nil {
		return true
	}
	if a == nil || b == nil {
	...
	if len(a.Origins) != len(b.Origins) || len(a.ResponseHeaders) != len(b.ResponseHeaders) {
		return false
	}
	...
	for i := range a.ResponseHeaders {
		if a.ResponseHeaders[i] != b.ResponseHeaders[i] {
			return false
		}
	}
	return true
...
//
// No-op: if the effective config is unchanged, the original pointer is returned
// (preserving nil vs non-nil identity).
func PatchCORS(base *storage.CORSConfig, addChunks []string, removeChunks []string) (*storage.CORSConfig, bool, error) {
	...
	if len(addChunks) > 0 {
		...
		if err != nil {
			return nil, false, fmt.Errorf("add cors: %w", err)
		}
	...
	if len(removeChunks) > 0 {
		...
		if err != nil {
			return nil, false, fmt.Errorf("delete cors: %w", err)
		}
		if rmMask.MaxAge {
			return nil, false, fmt.Errorf("delete-cors: %s cannot be specified; use --cors to set it", corsKeyMaxAge)
		}
	...
	// Deterministic (already sorted above), no-op detection
	if equalCORS(base, work) {
		return base, false, nil
	}
...
func PatchCustomHostnames(base []string, clear bool, add []string, remove []string) ([]string, bool, error) {
	var set StringSet
	if clear {
		set = make(StringSet)
	} else {
	...
	// no-op detection (order agnostic)
	if equalAsSets(base, out) {
		return base, false, nil
	}
	return out, true, nil
}
