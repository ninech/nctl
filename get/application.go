...
func printApplication(apps []apps.Application, get *Cmd, out io.Writer, header bool) error {
	...
	if header {
		get.writeHeader(w, "NAME", "REPLICAS", "HOSTS", "UNVERIFIEDHOSTS")
	}
	...
	for _, app := range apps {
		...
		get.writeTabRow(w, app.Namespace, app.Name, fmt.Sprintf("%d", replicas), join(verifiedHosts), join(unverifiedHosts))
	}
...
func joinUnverified(list []string) string {
	return strings.Join(list, "(unverified),")
}
