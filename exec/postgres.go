...
func (cmd *postgresCmd) Run(ctx context.Context, client *api.Client) error {
	...
	return connectAndExec(ctx, client, pg, postgresConnector{}, cmd.serviceCmd)
}
...
func (postgresConnector) Args(pg *storage.Postgres, user, pw string) ([]string, func(), error) {
	return psqlArgs(pg.Status.AtProvider.FQDN, "postgres", pg.Status.AtProvider.CACert, user, pw)
}
