...
func (cmd *mysqlCmd) Run(ctx context.Context, client *api.Client) error {
	...
	return connectAndExec(ctx, client, my, mysqlConnector{}, cmd.serviceCmd)
}
...
func (mysqlConnector) Args(my *storage.MySQL, user, pw string) ([]string, func(), error) {
	return mysqlArgs(my.Status.AtProvider.FQDN, "", my.Status.AtProvider.CACert, user, pw)
}
