	...
	cases := map[string]struct {
		...
		"all field updates": {
			...
			checkApp: func(t *testing.T, cmd applicationCmd, orig, updated *apps.Application) {
				...
				assert.Equal(t, util.UpdateEnvVars(existingApp.Spec.ForProvider.Config.Env, cmd.Env, cmd.DeleteEnv), updated.Spec.ForProvider.Config.Env)
				assert.Equal(t, util.UpdateEnvVars(existingApp.Spec.ForProvider.BuildEnv, cmd.BuildEnv, cmd.DeleteBuildEnv), updated.Spec.ForProvider.BuildEnv)
