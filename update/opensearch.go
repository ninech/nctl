...
type openSearchCmd struct {
	resourceCmd
	MachineType             *string          `help:"MachineType configures OpenSearch to use a specified machine type." placeholder:"e.g. nine-search-m"`
...
func (cmd *openSearchCmd) applyUpdates(openSearch *storage.OpenSearch) error {
	...
	if cmd.PublicNetworkingEnabled != nil {
		openSearch.Spec.ForProvider.PublicNetworkingEnabled = cmd.PublicNetworkingEnabled
	}
